...
% TODO(Dylan): insert your GitHub link.
The entire implementation can be found in the GitHub link below: \texttt{[INSERT LINK]}
...
% TODO(Dylan): figure placeholder -- commented out below since the plot
% (mid / best bid / best ask over the testing episode) hasn't been
% generated yet; a rendered "[FIGURE NOT YET GENERATED]" caption can't go
% out in a submission. Re-run the simulator with event-by-event logging
% and drop in figures/market_environment_timeseries.png, then uncomment.
% \begin{figure}[h]
% \centering
% \includegraphics[width=0.8\textwidth]{figures/market_environment_timeseries.png}
% \caption{Mid-price, best bid, and best ask over the course of the testing episode.}
...
% TODO(Dylan): this is the section your outline calls "what it means,
% limitations, honestly stated" -- the Limitations subsection below is
% filled in from your draft, but the interpretive discussion of *why* the
% basic agent collapses to never trading, and why the consolidated agent's
% behavior differs, is something you should write in your own words. You
% and I talked through one candidate explanation in our conversation (the
% basic agent's state is too impoverished to distinguish good quoting
% moments from adverse-selection-heavy ones, so its value estimate for
% quoting at all is pulled to roughly zero/negative, making "do nothing"
% a stable fixed point of the SARSA update) -- flagging it here as a
% pointer to that conversation, not as inserted prose, since you said you
% want to be able to defend everything in the final draft yourself.
